...
std::string TextNormalizer::normalize_vietnamese_text(const std::string& input_text) {
    ...
    // H. Tiền tệ có dấu chấm phân cách hàng ngàn: 1.250.000 hoặc 500.000
    {
        std::regex re(R"(\b\d{1,3}(?:\.\d{3})+\b)");
        std::string res;
        auto it = std::sregex_iterator(text.begin(), text.end(), re);
        auto end = std::sregex_iterator();
        size_t last = 0;
        for (; it != end; ++it) {
            ...
            try {
                int64_t val = std::stoll(clean_num);
    ...
    // I. Nhiệt độ: 36.5°C hoặc 36.5 độ C
    {
    ...
    // J. Tiền tệ viết tắt: 100k, 100tr, 100.000đ, $50
    {
        text = std::regex_replace(text, std::regex(R"((\d+)\s*k(?=[\s\.,;!?:/\-\)]|$))", std::regex_constants::icase), "$1 ngàn");
        text = std::regex_replace(text, std::regex(R"((\d+)\s*tr(?=[\s\.,;!?:/\-\)]|$))", std::regex_constants::icase), "$1 triệu");
        text = std::regex_replace(text, std::regex(R"((\d+)\s*đ(?=[\s\.,;!?:/\-\)]|$))", std::regex_constants::icase), "$1 đồng");
        text = std::regex_replace(text, std::regex(R"(\$\s*(\d+))"), "$1 đô la");
        text = std::regex_replace(text, std::regex(R"((\d+)\s*\$)"), "$1 đô la");
    }

    // K. Số thập phân: 1.5 hoặc 1,5
    {
    ...
    // L. Quy đổi ký hiệu & thuật ngữ nước ngoài trước (để 16GB -> 16 ghi ga bai)

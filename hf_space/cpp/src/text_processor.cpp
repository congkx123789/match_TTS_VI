...
std::vector<std::string> TextProcessor::split_sentences(const std::string& text) const {
    ...
    // Gộp các câu hoàn chỉnh thành các chunk chuẩn ~30 giây âm thanh (khoảng 480 - 520 ký tự)
    // Giúp nhịp đọc tự nhiên, cân bằng hoàn hảo giữa ngữ điệu và tốc độ sinh
    std::vector<std::string> chunks;
    std::string accumulated;
    const size_t TARGET_CHARS = 500; // ~30 giây âm thanh / chunk
    const size_t MIN_CHARS = 320;
